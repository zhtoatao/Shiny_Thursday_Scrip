...
def screen_shot():
    ...
    screenshot_file = os.path.join(current_folder, "screenshot", "screenshot2.png")
...
def co(target_img,
    ...
    target = os.path.join(current_folder, "target_non_stop", target_img)
    ...
    if max_val >= threshold:
        ...
        log = log_creater.write_log(f"发现异常 {target_img} 力,{done}")
        print(log)
        if click_:
...
def sc(target_img,
    ...
    min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
    if max_val >= threshold:
        log = log_creater.write_log(f"！！！发现异常 {target_image} ,等待 {sleeptime}秒")
        time.sleep(sleeptime)
...
def cos(target_img,
    ...
    min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
    if max_val >= threshold:
        log = log_creater.write_log(f"！！！发现异常 {target_img} 力，执行操作：偏移点击")
        print(log)
...
def sco(target_img,
    ...
    min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
    if max_val >= threshold:
        log = log_creater.write_log(f"！！！！！！发现异常 {target_img} 力,执行操作：直接点击")
        print(log)
        center_x = max_loc[0] + target_image.shape[1] // 2
        center_y = max_loc[1] + target_image.shape[0] // 2
        click(center_x,center_y,sleeptime)
...
# check once while,没有发现异常的话就会一直找
def cow(target_img,
       click_ = True,
       sleeptime=2,
       threshold=0.8,
